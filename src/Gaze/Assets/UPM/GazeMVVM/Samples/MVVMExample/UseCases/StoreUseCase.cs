...
namespace Gaze.MVVM.Example.UseCases
{
    [CreateAssetMenu(menuName = "Samples/MVVM/Use Cases/"+nameof(StoreUseCase))]
    public class StoreUseCase : ObservableScriptableObject
    {
        [SerializeField]
        ...
        {
            if (walletUseCase.ConsumeHardCurrency(storeModel.HardCurrencyCurrentConsumption))
            {
        ...
        {
            if (walletUseCase.ConsumeSoftCurrency(storeModel.HardCurrencyCurrentConsumption))
            {
        ...
        {
            walletUseCase.AddHardCurrency(HardCurrencyPackSize);
        }
        
        public void BuySoftCurrency()
        {
            walletUseCase.AddSoftCurrency(SoftCurrencyPackSize);
        }
    }
}
